...
<script>
import { listHall, getHall, delHall, addHall, updateHall } from "@/api/system/hall";
...
export default {
  ...
  methods: {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除hall编号为"' + ids + '"的数据项？').then(function() {
        return delHall(ids);
      }).then(() => {
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('system/hall/export', {
        ...this.queryParams
